  ...
  <div class="page-container">
    ...
    <el-dialog :title="operation?'新增':'编辑'" width="40%" :visible.sync="dialogVisible" :close-on-click-modal="false">
      <el-form :model="dataForm" label-width="80px" :rules="dataFormRules" ref="dataForm" :size="this.tableTheme.size" label-position="right">
        <el-form-item label="Code" prop="secondCategory" v-if="false">
          <el-input v-model="dataForm.secondCategory" :disabled="true" auto-complete="off"></el-input>
        </el-form-item>
        <el-form-item label="分类名称" prop="name">
          <el-input v-model="dataForm.name" maxlength="50" auto-complete="off"></el-input>
        </el-form-item>
        <el-form-item label="上级分类" prop="topCode">
          <el-select v-model="dataForm.topCode" placeholder="请选择">
				    <el-option
				      v-for="item in TopCategoryPageList"
				      :key="item.topCode"
				      :label="item.name"
				      :value="item.topCode">
				    </el-option>
...
export default {
  ...
  data() {
    return {
      ...
      dataFormRules: {
        ...
        topCode: [{ required: true, message: "请选择一级分类", trigger: "blur" }] 
      },
      //新增编辑界面数据
      dataForm: {
        topCode: "",
  ...
  methods: {
    ...
    findPage: function(data) {
      ...
      this.$api.product.SecondCategoryPage(this.pageRequest)       
      .then(res => {
        this.pageResult = res; 
      })
    ...
    handleDelete: function() {
      ...
      this.$confirm("删除该分类会同时删除相关联的产品，确认删除选中记录吗？", "提示", {
        type: "warning"
      }).then(() => {
        let params = [];
        for (var i = 0; i < selectData.length; i++) {
          params.push({ ID: selectData[i].secondCategory });
        }
        this.loading = true;
        this.$api.product.DeleteSecondCategory(params)
          .then(res => {
        ...
        if (valid) {
          this.$confirm("确认提交吗？", "提示", {}).then(() => {
            ...
            let params = Object.assign({}, this.dataForm);
          	this.$api.product.CreateOrUpdateSecondCategory(params)
            .then(res => {
    ...
    GetTopCategoryPage(){
    	...
    	this.$api.product.TopCategoryPage(TopCategoryPage)    
	  	.then( data => {	
	  		this.TopCategoryPageList = data.items;
	  	})
	  	.finally(res => {
        this.editLoading = false;
      })
    ...
    initColumns: function() {
      this.columns = [			
        ...
        { prop: "topCategoryName", label: "上级分类", minWidth: 120 },
        { prop: "picturePath", label: "分类图片", minWidth: 80, special: "img_picture" }
      ];
