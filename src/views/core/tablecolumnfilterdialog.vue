<template>
  ...
  <el-dialog title="表格显示列" width="40%" :visible.sync="dialogVisible" :close-on-click-modal="false">
    <el-table ref="fitlerTable" :data="columns" height="330px" tooltip-effect="dark" header-align="left" align="left" :size="size" style="width: 100%" @selection-change="selectionChange">
      ...
      this.$nextTick(function() {
        for (let i = 0; i <= this.columns.length; i++) {
          if (!this.columns[i].hide)
            _this.$refs.fitlerTable.toggleRowSelection(this.columns[i], true);
        }
      });
    },
    // 处理表格列过滤显示
