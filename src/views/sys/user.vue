  ...
  <div class="page-container">
    ...
    <div class="toolbar left">
      <el-form :inline="true" :model="filters" :size="this.tableTheme.size" @submit.native.prevent>
        ...
        <el-form-item>
          <el-input v-model="filters.name" @keyup.enter.native="findPage(null)" placeholder="姓名"></el-input>
        </el-form-item>
        <el-form-item>
    ...
    <!--表格内容栏-->
    <kt-table  ref="kttable" :headerAlign="this.tableTheme.headerAlign" :align="this.tableTheme.align" :stripe="this.tableTheme.stripe" :size="this.tableTheme.size" :border="this.tableTheme.border" :data="pageResult" :columns="filterColumns" @findPage="findPage"></kt-table>
    <!--新增编辑界面-->
    <el-dialog :title="operation?'新增':'编辑'" width="40%" :visible.sync="dialogVisible" :close-on-click-modal="false">
      <el-form :model="dataForm" label-width="80px" :rules="dataFormRules" ref="dataForm" :size="this.tableTheme.size" label-position="right">
        <el-form-item label="ID" prop="operatorID" v-if="false">
          <el-input v-model="dataForm.operatorID" :disabled="true" auto-complete="off"></el-input>
        </el-form-item>
        ...
        <el-form-item label="角色" prop="userRoles" v-if="!operation">
          <el-select v-model="dataForm.userRoles" multiple placeholder="请选择" style="width: 100%;">
            <el-option v-for="item in roles" :key="item.rolesID" :label="item.rolesName" :value="item.rolesID"></el-option>
          </el-select>
        </el-form-item>
      </el-form>
  ...
  methods: {
    ...
    handleDelete: function() {
      ...
      this.$confirm("确认删除选中记录吗？", "提示", {
        ...
        for (var i = 0; i < selectData.length; i++) {
          params.push({ id: selectData[i].operatorID });
        }
        this.loading = true;
        this.$api.sys_user
          .batchDelete(params)
    ...
    handleEdit: function() {
     let selectData = this.$refs.kttable.selections;
      ...
      let userRoles = [];
      for (let i = 0, len = selectData[0].userRoles.length; i < len; i++) {
        userRoles.push(selectData[0].userRoles[i].rolesID);
      }
    ...
    findDeptTree: function() {
      this.$api.sys_dept.findDeptTree().then(res => {
        this.deptData = res.items;
      });
    },
    // 菜单树选中
    ...
    initColumns: function() {
      this.columns = [
        { prop: "operatorID", label: "ID", minWidth: 80,hide:true },
        { prop: "operatorName", label: "姓名", minWidth: 120  },
        { prop: "loginName", label: "用户名", minWidth: 120 },
        { prop: "deptName", label: "机构", minWidth: 120 },
        { prop: "rolesName", label: "角色", minWidth: 100 }
      ];
